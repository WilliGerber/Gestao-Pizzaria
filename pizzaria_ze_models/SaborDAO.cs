...
public class SaborDAO
{
    ...
    public int Inserir(Sabor sabor)
    {
        ...
        try
        {
            ...
            foreach (Ingredient auxIngrediente in sabor.SaborIngredientes)
            {
    ...
    public DataTable Buscar(Sabor sabor)
    {
        ...
        "(SELECT GROUP_CONCAT(i.descricao_ingrediente SEPARATOR ', ') FROM itens_sabores AS iss, cad_ingredientes i WHERE iss.sabor_id = s.id_sabor AND i.id_ingrediente = iss.ingrediente_id ) AS Ingredientes " + "FROM tb_sabores AS s " +
        auxSqlFiltro +
